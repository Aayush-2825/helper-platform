...
export const helperPresence = pgTable(
  'helper_presence',
  {
    id: text('id').primaryKey(),
  ...
  (table) => [
    uniqueIndex('helperPresence_helperUserId_uidx').on(table.helperUserId),
    index('helperPresence_status_idx').on(table.status),
  ],
);
...
export const incomingJobs = pgTable(
  'incoming_jobs',
  {
    id: text('id').primaryKey(),
    bookingId: text('booking_id').notNull(),
