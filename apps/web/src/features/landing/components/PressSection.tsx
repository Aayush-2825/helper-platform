    ...
    <section className="py-16 px-6 border-t border-border bg-background">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {PRESS.map((p) => (
            <div key={p.name} className="flex flex-col items-center gap-3 p-5 rounded-lg border border-border bg-background hover:shadow-sm hover:border-border/80 transition-all group cursor-pointer">
              <div className={`size-12 rounded-md flex items-center justify-center font-black text-lg ${p.color} border border-current/10`}>
                {p.logo}
              </div>
              <p className="font-black text-sm text-foreground text-center">{p.name}</p>
              <p className="text-xs text-muted-foreground font-medium text-center leading-relaxed line-clamp-2">{p.quote}</p>
            </div>
          ))}
        </div>
      </div>
