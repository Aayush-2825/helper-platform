    ...
    <section className="py-24 px-6 border-t border-border bg-muted/20">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-2 text-sm font-bold text-muted-foreground bg-background border border-border px-4 py-2.5 rounded-md shrink-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {CITIES.map((city) => (
            <div
              key={city.name}
              className={`rounded-lg p-4 flex flex-col gap-2 border transition-all relative overflow-hidden ${
                city.status === "active"
            ...
            >
              {city.status === "active" && (
                <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500/10 rounded-bl-4xl" />
              )}
              ...
              {city.badge && (
                <span className="self-start text-xs font-black uppercase tracking-wide px-2 py-0.5 rounded-full bg-emerald-600 text-white">
                  {city.badge}
                </span>
              )}
            </div>
        ...
        <div className="bg-background border border-border rounded-xl p-6 flex flex-col sm:flex-row items-center gap-5 justify-between">
          ...
          <form onSubmit={handleNotify} className="w-full sm:w-auto shrink-0">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              />
              <Button type="submit" className="px-5 py-2.5 bg-foreground text-background font-bold text-sm rounded-md hover:bg-foreground/90 shrink-0">
                Notify Me
              </Button>
            </div>
