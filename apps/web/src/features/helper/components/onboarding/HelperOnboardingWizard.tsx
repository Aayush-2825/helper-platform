...
export function HelperOnboardingWizard({
  ...
  const validateStep = async (step: number, shouldFocus = false) => {
    ...
    switch (step) {
      ...
      case 4: {
        if (!values.dpdpConsentGiven || !hasText(values.dpdpConsentAt) || !hasText(values.dpdpConsentVersion)) {
          addError("dpdpConsentGiven", "DPDP consent is required before document upload.");
        }
    ...
    const handleBeforeUnload = () => {
      ...
      try {
        ...
        navigator.sendBeacon(
          "/api/helpers/onboarding/draft",
          new Blob([payload], { type: "application/json" }),
        );
      } catch {
        // Ignore unload persistence errors and rely on regular autosave.
      }
    };
